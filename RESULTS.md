...
| A23 | **partial** | Output gates pass: `lib/ml/gpu_ops.zeph` plus `tests/ml/gpu_ops_test.zeph`, 95 checks over 47 comparisons at 1e-12 relative, covering all four binary ops, thirteen unary ops, `sum_dim` on every axis, gather (with `roll` and `concat` expressed through it) and scatter-add with repeated indices, in both dtypes and at ranks 1, 2 and 3. transpose, slice, reshape and expand are metadata under the stride contract and are tested by feeding non-contiguous views into the other kernels. Mutation-tested. **Still owed: the gradient and update gates.** The criterion asks for every op supported with its VJP and for CPU/GPU gradient and optimizer-update agreement, and neither is wired yet. Complex transcendentals are device-only, since `t_map1` panics on complex, and are not claimed. |
| A24, A25, A32, A33, A40, A41 | **not_run** | Not started. The specification defines no A15–A19, A26–A29 or A34–A39; the 27 defined IDs are A00–A14, A20–A25, A30–A33, A40 and A41. |
