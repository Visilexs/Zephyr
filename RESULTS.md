...
| A21 | **blocked** | The Vulkan arm is complete. `tools/gpu_step_bench.zeph` runs the same full training step on both paths and reports correctness first: the loss agrees to 3e-15 and the gradients agree across 104,160 components to 4e-14 at the specification size. Warm median and p95, cold pipeline build, host transfer rate, peak device memory and fp32 against fp64 are all in `.agent/env-ledger.md`. **No forward-only winner is reported as a winner:** at D=8 the backend was 1.01x ahead on the forward pass and 0.98x behind on the full step, and the benchmark prints that verdict explicitly; at D=128 it was 7.79x ahead on the forward pass and 5.43x on the full step, so the win narrowed by about 40% once the backward was included. **Those figures are superseded and the conclusion is reversed.** They were measured against the pre-M6b CPU path. After the A25 optimizations made the CPU step 14.3x faster the same benchmark, unchanged, now reports the device losing at both sizes: at D=128, forward 164 ms CPU against 290 ms device (0.57x) and full step 585 ms against 1298 ms (0.45x), and it prints VERDICT: loses at this size. The device kernels were not made slower; the baseline they were beating was slow. Agreement is unaffected -- loss to 3e-15 and 104,160 gradient components to 4e-14 -- so this is a statement about which path is faster, not about whether the backend is correct. The M7 training runs therefore use the CPU path. **The CUDA arm cannot be run on this machine** -- NVRTC, cuBLAS and the CUDA toolkit are all absent -- so the Vulkan-against-CUDA comparison this criterion asks for does not exist and is not claimed. |
...
| A40 | **passed** | `lib/ml/causal.zeph` and `tests/ml/causal_test.zeph`, 20 checks. The classification path runs its K THINK steps once at the end, which cannot support generation, so causal mode defines its own policy -- consume a token, then run K THINK steps, per position -- and that policy is stated at the top of the module because it is what makes generation and training agree. Four properties are checked as exact identities rather than tolerances, since all four are exact arithmetic: chunked execution with a carried state is **bit-identical** to running the sequence whole; a reset makes the remainder bit-identical to running it from scratch and independent of any prefix before it; every greedily generated token is exactly the argmax teacher forcing reads at that position; and with the readout zeroed the model is exactly uniform, so perplexity equals the vocabulary size exactly, which is what "computed from valid logp" requires if padding is genuinely excluded. Position counts are asserted directly (5 of 12 on a padded batch, 10 of 12 on a full one). Gradients agree with central differences to 5.6e-10, and the model learns a repeating pattern from perplexity 17.7 to 1.65. Three rejection cases run in their own processes. |
| A41 | **passed** | `tests/ml/lifecycle_test.zeph`, 11 checks. One native binary trains, evaluates, checkpoints, resumes and generates, linked against kernel32 only; no Python is involved at any point and the Python oracle remains available and unchanged for the parity work it already backs. Resume equivalence is the load-bearing check and it is bit-exact across all eight parameters, both AdamW moment buffers and the step counter -- a resume that dropped the moments would still train and still look plausible. Generation after the resume emits the identical token sequence, because a model that is numerically restored but generates differently has a broken generation path rather than a broken checkpoint, and a vacuity check confirms the intervening steps actually moved the model. A second save/load round trip is bit-exact and the re-saved payload is required to differ byte-for-byte from the first. **The op-coverage ledger is now project-wide** (`tools/op_census.zeph`): it runs the phase classifier, both ablations, the unfused path, the causal path, both GRU heads and the Transformer, and reports per family and unioned. 29 of 31 opcodes are reached; the two that are not are named and explained rather than merely listed -- LEAF is never a node by construction, and COMPLEX is genuinely unused because no family builds a complex value from separate real and imaginary tensors. The census used to crash with a bare "index out of bounds" on an unknown opcode and now names it and says a new operator was added without being added to the ledger. |
| A32, A33 | **not_run** | The apparatus is complete and verified; no sweep has been run and no result is claimed. Present: frozen datasets with recorded hashes (`data/m7/`, 98-check loader), every control the baselines document names -- GRU state-matched, parameter-matched and quadratic-head (275 checks), a causal Transformer (18), both mechanism ablations (19) -- a single-run training harness whose entire budget is a block of constants copied into each run record, a resumable sweep driver, and a report generator that separates measurement from interpretation and renders missing metrics as "n/a" rather than zero. The declared preset is **deliberately below** the difficulty the specification suggests (task A n=2..4, d=0..8 against the suggested n=2..8, d=0..32; 1,000 steps against a suggested cap of 10,000) because the full preset is about 300 hours on this machine; the reduction is recorded in the dataset manifest and in every run record. One pilot run completed in 8m47s and reached validation accuracy 0.484 against chance 0.125, which establishes that the pipeline trains and nothing more. **That pilot read the frozen composition and length splits for one configuration before the protocol had been exercised**, which A33 forbids; it is disclosed here rather than omitted, a `smoke` mode now exists so a pipeline check cannot spend the frozen splits again, and the honest remedy before any confirmatory sweep is to regenerate the test splits under a new generation seed, which `freeze_datasets.py` does deterministically. |
